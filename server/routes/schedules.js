...
class Schedules {
  ...
  initializeRoutes(express) {
    ...
    router.get("/:teamName", (req, res, next) =>
      this.getTeamSchedule(req, res, next)
    );
    router.put("/:teamName", (req, res, next) =>
      this.putTeamSchedule(req, res, next)
    );
    return router;
  }
  ...
  getTeamSchedule(req, res, next) {
    ...
    var currentSchedules = this.storage.getSchedules(
      startDate,
      noOfDays,
      teamName
    );
    let schedules = [];
    _.forEach(_.range(0, noOfDays), day => {
      let date = new Date(startDate.valueOf());
      date.setDate(date.getDate() + day);
      let matchingSchedule = _.find(currentSchedules, currentSchedule => {
        return currentSchedule.day.getFullYear() == date.getFullYear() && currentSchedule.day.getMonth() == date.getMonth() && currentSchedule.day.getDate() == date.getDate();
      });
      if (matchingSchedule) {
        schedules.push(matchingSchedule);
      } else {
        schedules.push(this.createDummySchedule(date));
      }
    });
    res.send(schedules);
  }
  ...
  putTeamSchedule(req, res, next) {
    ...
    let scheduleError = _.find(schedules, schedule => {
      ...
      if (
        schedule.schedules[0].from !== null ||
        schedule.schedules[0].to !== null
      ) {
        res
          .status(400)
          .send(
            "schedule's from and to should be null till this feature is implemented"
          );
        return true;
      }
      ...
      if (
        isPast(schedule.day) &&
        this.getDayDiff(schedule.day, currentDate) < 0
      ) {
    ...
    _.remove(currentSchedules, currentSchedule => {
      return _.find(schedules, newSchedule =>
        isEqual(newSchedule.day, currentSchedule.day)
      );
    });
